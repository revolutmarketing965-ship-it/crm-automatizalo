...
export default function CRMPage() {
  ...
  // Estados de carga y modales de soporte para los componentes
  ...
  const handleCreateOrUpdateLead = async (e: React.FormEvent) => {
    ...
    // GARANTIZAR EMPRESA_ID OBTENIDO DEL PERFIL ACTUAL
    const empresaId = userProfile?.empresa_id;
    if (!empresaId) {
      alert('Error: El usuario actual no tiene una sucursal asignada.');
      setSavingLead(false);
      return;
    }
    ...
    if (editingLead) {
      ...
      const payload: any = { 
        ...
        status: leadStatus,
        notes: notasLead || null,
        empresa_id: empresaId // INYECCIÓN ESTRICTA
      };
  ...
  const handleCreateOrUpdateCita = async (e: React.FormEvent) => {
    ...
    const empresaId = userProfile?.empresa_id;
    if (!empresaId) {
      alert('Error: El usuario no tiene sucursal asignada.');
      setSavingCita(false);
      return;
    }
  ...
  const handleConvertLeadToSocio = async (leadId: string) => {
    ...
    const empresaId = userProfile?.empresa_id;
    if (!empresaId) {
      alert('Error: Usuario sin sucursal asignada.');
      return;
    }
